...
import { Request, RequestDocument } from 'src/_request/Request.schema'
import { Status } from 'src/enums/RequestType'
import { IRequestGetByFieldDto, IRequestLatest } from 'src/interfaces/Request'

@Injectable()
export class RequestRepository {
	constructor(@InjectModel('requests') private readonly requestModel: SoftDeleteModel<RequestDocument>) {}
	...
	async findByField(body: IRequestGetByFieldDto): Promise<Request[]> {
		...
		const data: Request[] = await this.requestModel
			.find(filter)
			.sort({
				createdAt: -1
			})
	...
	async findOne(id: mongoose.Types.ObjectId): Promise<Request> {
		const data: Request = await this.requestModel.findOne({ _id: id }).lean()
		return data
	}
